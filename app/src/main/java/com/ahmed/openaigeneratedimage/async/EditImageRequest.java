...
public class EditImageRequest {
    ...
    public EditImageRequest(Context ctx){
        this.ctx = ctx;
    }

    public String editImage(String prompt, Bitmap image) {
        ...
        image.compress(Bitmap.CompressFormat.PNG, 100, baos0);
        byte[] imageBytes0 = baos0.toByteArray();

        try {
            js.put("prompt", prompt);
            js.put("image", Base64.encodeToString(imageBytes0, Base64.DEFAULT));
            js.put("n", 1);
        ...
        JsonObjectRequest request = new JsonObjectRequest(Request.Method.POST, Constants.EDIT_IMAGE_URL, js, new Response.Listener<JSONObject>() {
            @Override
                ...
                if (!response.equals(null)) {
                    ...
                    try {
                        ...
                        Glide.with(ctx).
                                ...
                                .fitCenter()
                                .into(EditImageFragment.editImageGeneratedImg);
